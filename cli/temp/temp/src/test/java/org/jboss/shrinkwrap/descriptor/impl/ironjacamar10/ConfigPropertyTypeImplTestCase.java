...
{   
   ...
   @Test
   public void testConfigPropertyType() throws Exception
   {
      TestDescriptorImpl provider = new TestDescriptorImpl("test");
      ConfigPropertyType<TestDescriptorImpl> type = new ConfigPropertyTypeImpl<TestDescriptorImpl>(provider, "config-propertyType", provider.getRootNode());
      type.configPropertyType("test");
      assertEquals(type.getConfigPropertyType(), "test");
      type.removeConfigPropertyType();
      assertNull(type.getConfigPropertyType());
   }
}
