...
public class DataSourceTypeImpl<T> implements Child<T>, DataSourceType<T> {
	...
	public PropertyType<DataSourceType<T>> property() {
		return new PropertyTypeImpl<DataSourceType<T>>(this, "property",
				childNode);
	}
	...
	public List<PropertyType<DataSourceType<T>>> getPropertyList() {
		...
		for (Node node : nodeList) {
			PropertyType<DataSourceType<T>> type = new PropertyTypeImpl<DataSourceType<T>>(
	...
	public IsolationLevelType getIsolationLevel() {
		return IsolationLevelType.getFromStringValue(childNode
				.textValue("isolation-level"));
	}
