 ...
 * 
 * Original Documentation: The method-attributes element can be used to specify
 * which methods are read only or idempotent. This is used to reduce the need
 * for locks and replication.
 * 
 * 
 */
public class MethodAttributesTypeImpl<T> implements Child<T>,
	...
	public MethodType<MethodAttributesType<T>> method() {
		return new MethodTypeImpl<MethodAttributesType<T>>(this, "method",
				childNode);
	}
	...
	public List<MethodType<MethodAttributesType<T>>> getMethodList() {
		...
		for (Node node : nodeList) {
			MethodType<MethodAttributesType<T>> type = new MethodTypeImpl<MethodAttributesType<T>>(
