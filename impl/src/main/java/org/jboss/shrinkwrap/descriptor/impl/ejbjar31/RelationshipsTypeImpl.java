 ...
 * 
 * 
 * The relationshipsType describes the relationships in which entity beans with
 * container-managed persistence participate. The relationshipsType contains an
 * optional description; and a list of ejb-relation elements, which specify the
...
public class RelationshipsTypeImpl<T> implements Child<T>, RelationshipsType<T> {
	...
	public EjbRelationType<RelationshipsType<T>> ejbRelation() {
		return new EjbRelationTypeImpl<RelationshipsType<T>>(this,
				"ejb-relation", childNode);
	}
	...
	public List<EjbRelationType<RelationshipsType<T>>> getEjbRelationList() {
		...
		for (Node node : nodeList) {
			EjbRelationType<RelationshipsType<T>> type = new EjbRelationTypeImpl<RelationshipsType<T>>(
					this, "ejb-relation", childNode, node);
			list.add(type);
		}
		return list;
	}
