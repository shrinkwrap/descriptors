 ...
 * client. If not specified, it defaults to false. - an optional inherited
 * element. If this element is set to true, subclasses of the exception class
 * type are also automatically considered application exceptions (unless
 * overriden at a lower level). If set to false, only the exception class type
 * is considered an application-exception, not its exception subclasses. If not
 * specified, this value defaults to true.
 * 
 * 
 * 
 */
public class ApplicationExceptionTypeImpl<T> implements Child<T>,
