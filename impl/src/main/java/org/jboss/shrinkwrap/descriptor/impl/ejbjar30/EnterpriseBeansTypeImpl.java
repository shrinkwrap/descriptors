 ...
 * 
 * 
 * The enterprise-beansType declares one or more enterprise beans. Each bean can
 * be a session, entity or message-driven bean.
 * 
 * 
 * 
 */
public class EnterpriseBeansTypeImpl<T> implements Child<T>,
	...
	public EntityBeanType<EnterpriseBeansType<T>> entity() {
		return new EntityBeanTypeImpl<EnterpriseBeansType<T>>(this, "entity",
				childNode);
	}
	...
	public List<EntityBeanType<EnterpriseBeansType<T>>> getEntityList() {
		...
		for (Node node : nodeList) {
			EntityBeanType<EnterpriseBeansType<T>> type = new EntityBeanTypeImpl<EnterpriseBeansType<T>>(
					this, "entity", childNode, node);
			list.add(type);
		}
		return list;
	}
