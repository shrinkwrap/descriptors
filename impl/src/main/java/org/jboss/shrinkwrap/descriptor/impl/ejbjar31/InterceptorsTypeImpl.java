 ...
 * components within this ejb-jar or .war. The declaration consists of :
 * 
...
public class InterceptorsTypeImpl<T> implements Child<T>, InterceptorsType<T> {
	...
	public InterceptorType<InterceptorsType<T>> interceptor() {
		return new InterceptorTypeImpl<InterceptorsType<T>>(this,
				"interceptor", childNode);
	}
	...
	public List<InterceptorType<InterceptorsType<T>>> getInterceptorList() {
		...
		for (Node node : nodeList) {
			InterceptorType<InterceptorsType<T>> type = new InterceptorTypeImpl<InterceptorsType<T>>(
					this, "interceptor", childNode, node);
			list.add(type);
		}
		return list;
	}
