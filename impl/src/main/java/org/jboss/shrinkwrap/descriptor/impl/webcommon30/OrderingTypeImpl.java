...
public class OrderingTypeImpl<T> implements Child<T>, OrderingType<T> {
	...
	public OrderingOrderingType<OrderingType<T>> after() {
		...
		OrderingOrderingType<OrderingType<T>> after = new OrderingOrderingTypeImpl<OrderingType<T>>(
				this, "after", childNode, node);
		return after;
	}
	...
	public OrderingOrderingType<OrderingType<T>> before() {
		...
		OrderingOrderingType<OrderingType<T>> before = new OrderingOrderingTypeImpl<OrderingType<T>>(
				this, "before", childNode, node);
		return before;
	}
