...
public class Persistence20DescriptorImportTestCase
{
   ...
   public void testImportTutorialPersistenceXml() throws Exception
   {
      ...
      assertEquals(persistence.getPersistenceUnitList().get(0).getTransactionType(), PersistenceUnitTransactionType._JTA);
      assertEquals(persistence.getPersistenceUnitList().get(0).getProvider(), "org.eclipse.persistence.jpa.PersistenceProvider");
      assertEquals(persistence.getPersistenceUnitList().get(0).getJtaDataSource(), "jdbc/sample");
      assertEquals(persistence.getPersistenceUnitList().get(0).properties().getPropertyList().get(0).getName(), "eclipselink.ddl-generation");
      assertEquals(persistence.getPersistenceUnitList().get(0).properties().getPropertyList().get(0).getValue(), "drop-and-create-tables");
   ...
   public void testCreateTutorialPersistenceXml() throws Exception
   {
      final Persistence20Descriptor persistence = create()
            ...
            .setVersion("2.0")
            .persistenceUnit()
               .setName("LeanServiceECBComponentPU")
               .setTransactionType(PersistenceUnitTransactionType._JTA)
               .setProvider("org.eclipse.persistence.jpa.PersistenceProvider")
               .setJtaDataSource("jdbc/sample")
               .properties()
                  .property()
                     .setName("eclipselink.ddl-generation")
                     .setValue("drop-and-create-tables")
                  .up()
               .up()
            .up();
