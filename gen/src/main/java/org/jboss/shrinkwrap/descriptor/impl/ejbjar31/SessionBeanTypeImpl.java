 ...
 * bean's local home and local interfaces, if any - an optional declaration that
 * this bean exposes a no-interface view - the name of the session bean's web
 * service endpoint interface, if any - the session bean's implementation class
 * - the session bean's state management type - an optional declaration of a
 * stateful session bean's timeout value - an optional declaration of the
 * session bean's timeout method for handling programmatically created timers -
 * an optional declaration of timers to be automatically created at deployment
 * time - an optional declaration that a Singleton bean has eager initialization
 * - an optional declaration of a Singleton/Stateful bean's concurrency
 * management type - an optional declaration of the method locking metadata for
 * a Singleton with container managed concurrency - an optional declaration of
 * the other Singleton beans in the application that must be initialized before
 * this bean - an optional declaration of the session bean's asynchronous
 * methods - the optional session bean's transaction management type. If it is
 * not present, it is defaulted to Container. - an optional declaration of a
 * stateful session bean's afterBegin, beforeCompletion, and/or afterCompletion
 * methods - an optional list of the session bean class and/or superclass
 * around-invoke methods. - an optional list of the session bean class and/or
 * superclass around-timeout methods. - an optional declaration of the bean's
...
public class SessionBeanTypeImpl<T> implements Child<T>, SessionBeanType<T> {
	...
	public DataSourceType<SessionBeanType<T>> dataSource() {
		return new DataSourceTypeImpl<SessionBeanType<T>>(this, "data-source",
				childNode);
	}
	...
	public List<DataSourceType<SessionBeanType<T>>> getDataSourceList() {
		...
		for (Node node : nodeList) {
			DataSourceType<SessionBeanType<T>> type = new DataSourceTypeImpl<SessionBeanType<T>>(
	...
	public StatefulTimeoutType<SessionBeanType<T>> statefulTimeout() {
		...
		StatefulTimeoutType<SessionBeanType<T>> statefulTimeout = new StatefulTimeoutTypeImpl<SessionBeanType<T>>(
				this, "stateful-timeout", childNode, node);
	...
	public List<TimerType<SessionBeanType<T>>> getTimerList() {
		...
		for (Node node : nodeList) {
			TimerType<SessionBeanType<T>> type = new TimerTypeImpl<SessionBeanType<T>>(
					this, "timer", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	// isComplexType: false maxOccurs: - isAttribute: false
	// -------------------------------------------------------------------------------------||
	public SessionBeanType<T> setConcurrencyManagementType(
			ConcurrencyManagementTypeType concurrencyManagementType) {
		childNode.getOrCreate("concurrency-management-type").text(
				concurrencyManagementType);
		return this;
	}

	public SessionBeanType<T> setConcurrencyManagementType(
			String concurrencyManagementType) {
		childNode.getOrCreate("concurrency-management-type").text(
				concurrencyManagementType);
		return this;
	}
	...
	public ConcurrencyManagementTypeType getConcurrencyManagementType() {
		return ConcurrencyManagementTypeType.getFromStringValue(childNode
				.textValue("concurrency-management-type"));
	}
	...
	public ConcurrentMethodType<SessionBeanType<T>> concurrentMethod() {
		return new ConcurrentMethodTypeImpl<SessionBeanType<T>>(this,
				"concurrent-method", childNode);
	}
	...
	public List<ConcurrentMethodType<SessionBeanType<T>>> getConcurrentMethodList() {
		...
		for (Node node : nodeList) {
			ConcurrentMethodType<SessionBeanType<T>> type = new ConcurrentMethodTypeImpl<SessionBeanType<T>>(
					this, "concurrent-method", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public DependsOnType<SessionBeanType<T>> dependsOn() {
		...
		DependsOnType<SessionBeanType<T>> dependsOn = new DependsOnTypeImpl<SessionBeanType<T>>(
				this, "depends-on", childNode, node);
		return dependsOn;
	}
	...
	public AsyncMethodType<SessionBeanType<T>> asyncMethod() {
		return new AsyncMethodTypeImpl<SessionBeanType<T>>(this,
				"async-method", childNode);
	}
	...
	public List<AsyncMethodType<SessionBeanType<T>>> getAsyncMethodList() {
		...
		for (Node node : nodeList) {
			AsyncMethodType<SessionBeanType<T>> type = new AsyncMethodTypeImpl<SessionBeanType<T>>(
					this, "async-method", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public NamedMethodType<SessionBeanType<T>> afterBeginMethod() {
		...
		NamedMethodType<SessionBeanType<T>> afterBeginMethod = new NamedMethodTypeImpl<SessionBeanType<T>>(
				this, "after-begin-method", childNode, node);
	...
	public NamedMethodType<SessionBeanType<T>> beforeCompletionMethod() {
		...
		NamedMethodType<SessionBeanType<T>> beforeCompletionMethod = new NamedMethodTypeImpl<SessionBeanType<T>>(
				this, "before-completion-method", childNode, node);
	...
	public NamedMethodType<SessionBeanType<T>> afterCompletionMethod() {
		...
		NamedMethodType<SessionBeanType<T>> afterCompletionMethod = new NamedMethodTypeImpl<SessionBeanType<T>>(
				this, "after-completion-method", childNode, node);
	...
	public AroundTimeoutType<SessionBeanType<T>> aroundTimeout() {
		return new AroundTimeoutTypeImpl<SessionBeanType<T>>(this,
	...
	public List<AroundTimeoutType<SessionBeanType<T>>> getAroundTimeoutList() {
		...
		for (Node node : nodeList) {
			AroundTimeoutType<SessionBeanType<T>> type = new AroundTimeoutTypeImpl<SessionBeanType<T>>(
