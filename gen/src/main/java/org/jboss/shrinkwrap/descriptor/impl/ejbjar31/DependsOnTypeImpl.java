 ...
 * 
 * 
 * The depends-onType is used to express initialization ordering dependencies
 * between Singleton components. The depends-onType specifies the names of one
 * or more Singleton beans in the same application as the referring Singleton,
 * each of which must be initialized before the referring bean.
 * 
 * Each dependent bean is expressed using ejb-link syntax. The order in which
 * dependent beans are initialized at runtime is not guaranteed to match the
