...
public class SessionConfigTypeImpl<T> implements Child<T>, SessionConfigType<T> {
	...
	public CookieConfigType<SessionConfigType<T>> cookieConfig() {
		...
		CookieConfigType<SessionConfigType<T>> cookieConfig = new CookieConfigTypeImpl<SessionConfigType<T>>(
				this, "cookie-config", childNode, node);
		return cookieConfig;
	}
	...
	public TrackingModeType getTrackingMode() {
		return TrackingModeType.getFromStringValue(childNode
				.textValue("tracking-mode"));
	}
