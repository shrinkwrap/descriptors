 ...
 * 
 * Original Documentation: The enterprise-beans element contains additional
 * information about the beans. These informations, such as jndi names, resource
 * managers and container configurations, are specific to jboss and not
 * described in ejb-jar.xml. jboss will provide a standard behaviour if no
 * enterprise-beans element is found, see container-configurations, jndi-name
 * and resource-managers for defaults. Used in: jboss
 * 
 */
public class EnterpriseBeansTypeImpl<T> implements Child<T>,
	...
	public SessionBeanType<EnterpriseBeansType<T>> session() {
		return new SessionBeanTypeImpl<EnterpriseBeansType<T>>(this, "session",
				childNode);
	}
	...
	public List<SessionBeanType<EnterpriseBeansType<T>>> getSessionList() {
		...
		for (Node node : nodeList) {
			SessionBeanType<EnterpriseBeansType<T>> type = new SessionBeanTypeImpl<EnterpriseBeansType<T>>(
					this, "session", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public MessageDrivenBeanType<EnterpriseBeansType<T>> messageDriven() {
		return new MessageDrivenBeanTypeImpl<EnterpriseBeansType<T>>(this,
				"message-driven", childNode);
	}
	...
	public ServiceBeanType<EnterpriseBeansType<T>> service() {
		return new ServiceBeanTypeImpl<EnterpriseBeansType<T>>(this, "service",
				childNode);
	}
	...
	public List<ServiceBeanType<EnterpriseBeansType<T>>> getServiceList() {
		...
		for (Node node : nodeList) {
			ServiceBeanType<EnterpriseBeansType<T>> type = new ServiceBeanTypeImpl<EnterpriseBeansType<T>>(
					this, "service", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public ConsumerBeanType<EnterpriseBeansType<T>> consumer() {
		return new ConsumerBeanTypeImpl<EnterpriseBeansType<T>>(this,
				"consumer", childNode);
	}
	...
	public List<ConsumerBeanType<EnterpriseBeansType<T>>> getConsumerList() {
		...
		for (Node node : nodeList) {
			ConsumerBeanType<EnterpriseBeansType<T>> type = new ConsumerBeanTypeImpl<EnterpriseBeansType<T>>(
					this, "consumer", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public GenericBeanType<EnterpriseBeansType<T>> ejb() {
		return new GenericBeanTypeImpl<EnterpriseBeansType<T>>(this, "ejb",
				childNode);
	}
	...
	public List<GenericBeanType<EnterpriseBeansType<T>>> getEjbList() {
		...
		for (Node node : nodeList) {
			GenericBeanType<EnterpriseBeansType<T>> type = new GenericBeanTypeImpl<EnterpriseBeansType<T>>(
					this, "ejb", childNode, node);
			list.add(type);
		}
		return list;
	}
