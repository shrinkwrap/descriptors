 ...
 * to the enterprise bean in the deployment descriptor - an optional mapped-name
 * element that can be used to provide vendor-specific deployment information
 * such as the physical jndi-name of destination from which this message-driven
 * bean should consume. This element is not required to be supported by all
 * implementations. Any use of this element is non-portable. - the
 * message-driven bean's implementation class - an optional declaration of the
 * bean's messaging type - an optional declaration of the bean's timeout method
 * for handling programmatically created timers - an optional declaration of
 * timers to be automatically created at deployment time - the optional
 * message-driven bean's transaction management type. If it is not defined, it
 * is defaulted to Container. - an optional declaration of the bean's
 * message-destination-type - an optional declaration of the bean's
 * message-destination-link - an optional declaration of the message-driven
 * bean's activation configuration properties - an optional list of the
 * message-driven bean class and/or superclass around-invoke methods. - an
 * optional list of the message-driven bean class and/or superclass
 * around-timeout methods. - an optional declaration of the bean's environment
...
public class MessageDrivenBeanTypeImpl<T> implements Child<T>,
	...
	public EnvEntryType<MessageDrivenBeanType<T>> envEntry() {
		return new EnvEntryTypeImpl<MessageDrivenBeanType<T>>(this,
				"env-entry", childNode);
	}
	...
	public List<EnvEntryType<MessageDrivenBeanType<T>>> getEnvEntryList() {
		...
		for (Node node : nodeList) {
			EnvEntryType<MessageDrivenBeanType<T>> type = new EnvEntryTypeImpl<MessageDrivenBeanType<T>>(
	...
	public PersistenceContextRefType<MessageDrivenBeanType<T>> persistenceContextRef() {
		return new PersistenceContextRefTypeImpl<MessageDrivenBeanType<T>>(
				this, "persistence-context-ref", childNode);
	}
	...
	public PersistenceUnitRefType<MessageDrivenBeanType<T>> persistenceUnitRef() {
		return new PersistenceUnitRefTypeImpl<MessageDrivenBeanType<T>>(this,
	...
	public LifecycleCallbackType<MessageDrivenBeanType<T>> postConstruct() {
		return new LifecycleCallbackTypeImpl<MessageDrivenBeanType<T>>(this,
	...
	public DataSourceType<MessageDrivenBeanType<T>> dataSource() {
		return new DataSourceTypeImpl<MessageDrivenBeanType<T>>(this,
				"data-source", childNode);
	}
	...
	public List<DataSourceType<MessageDrivenBeanType<T>>> getDataSourceList() {
		...
		for (Node node : nodeList) {
			DataSourceType<MessageDrivenBeanType<T>> type = new DataSourceTypeImpl<MessageDrivenBeanType<T>>(
	...
	public NamedMethodType<MessageDrivenBeanType<T>> timeoutMethod() {
		...
		NamedMethodType<MessageDrivenBeanType<T>> timeoutMethod = new NamedMethodTypeImpl<MessageDrivenBeanType<T>>(
	...
	public TimerType<MessageDrivenBeanType<T>> timer() {
		return new TimerTypeImpl<MessageDrivenBeanType<T>>(this, "timer",
				childNode);
	}
	...
	public List<TimerType<MessageDrivenBeanType<T>>> getTimerList() {
		...
		for (Node node : nodeList) {
			TimerType<MessageDrivenBeanType<T>> type = new TimerTypeImpl<MessageDrivenBeanType<T>>(
	...
	// isComplexType: false maxOccurs: - isAttribute: false
	// -------------------------------------------------------------------------------------||
	public MessageDrivenBeanType<T> setTransactionType(
	...
	// isComplexType: false maxOccurs: - isAttribute: false
	// -------------------------------------------------------------------------------------||
	public MessageDrivenBeanType<T> setMessageDestinationType(
	...
	// isComplexType: false maxOccurs: - isAttribute: false
	// -------------------------------------------------------------------------------------||
	public MessageDrivenBeanType<T> setMessageDestinationLink(
	...
	public AroundInvokeType<MessageDrivenBeanType<T>> aroundInvoke() {
		return new AroundInvokeTypeImpl<MessageDrivenBeanType<T>>(this,
	...
	public List<AroundInvokeType<MessageDrivenBeanType<T>>> getAroundInvokeList() {
		...
		for (Node node : nodeList) {
			AroundInvokeType<MessageDrivenBeanType<T>> type = new AroundInvokeTypeImpl<MessageDrivenBeanType<T>>(
	...
	public AroundTimeoutType<MessageDrivenBeanType<T>> aroundTimeout() {
		return new AroundTimeoutTypeImpl<MessageDrivenBeanType<T>>(this,
	...
	public List<AroundTimeoutType<MessageDrivenBeanType<T>>> getAroundTimeoutList() {
		...
		for (Node node : nodeList) {
			AroundTimeoutType<MessageDrivenBeanType<T>> type = new AroundTimeoutTypeImpl<MessageDrivenBeanType<T>>(
	...
	public SecurityRoleRefType<MessageDrivenBeanType<T>> securityRoleRef() {
		return new SecurityRoleRefTypeImpl<MessageDrivenBeanType<T>>(this,
