 ...
 * 
 * 
 * The cmr-fieldType describes the bean provider's view of a relationship. It
 * consists of an optional description, and the name and the class type of a
 * field in the source of a role of a relationship. The cmr-field-name element
 * corresponds to the name used for the get and set accessor methods for the
 * relationship. The cmr-field-type element is used only for collection-valued
...
public class CmrFieldTypeImpl<T> implements Child<T>, CmrFieldType<T> {
	...
	public CmrFieldTypeType getCmrFieldType() {
		return CmrFieldTypeType.getFromStringValue(childNode
				.textValue("cmr-field-type"));
	}
