 ...
 * etc.); the component and web service endpoint interfaces, and so on.
 * 
 * Local applies to the local component interface, local business interfaces,
 * and the no-interface view.
 * 
 * Remote applies to both remote component interface and the remote business
 * interfaces.
 * 
 * ServiceEndpoint refers to methods exposed through a web service endpoint.
 * 
...
public enum MethodIntfType {
	_HOME("Home"), _REMOTE("Remote"), _LOCALHOME("LocalHome"), _LOCAL("Local"), _SERVICEENDPOINT(
			"ServiceEndpoint"), _TIMER("Timer"), _MESSAGEENDPOINT(
			"MessageEndpoint");
