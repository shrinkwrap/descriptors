 ...
 * element that contains a small and a large icon file name - a unique name
 * assigned to the enterprise bean in the deployment descriptor - an optional
 * mapped-name element that can be used to provide vendor-specific deployment
 * information such as the physical jndi-name of the entity bean's remote home
 * interface. This element is not required to be supported by all
 * implementations. Any use of this element is non-portable. - the names of the
 * entity bean's remote home and remote interfaces, if any - the names of the
 * entity bean's local home and local interfaces, if any - the entity bean's
 * implementation class - the optional entity bean's persistence management
 * type. If this element is not specified it is defaulted to Container. - the
 * entity bean's primary key class name - an indication of the entity bean's
 * reentrancy - an optional specification of the entity bean's cmp-version - an
 * optional specification of the entity bean's abstract schema name - an
 * optional list of container-managed fields - an optional specification of the
 * primary key field - an optional declaration of the bean's environment entries
 * - an optional declaration of the bean's EJB references - an optional
 ...
 * declaration of the bean's message destination references - an optional set of
 * query declarations for finder and select methods for an entity bean with
 * cmp-version 2.x.
 * 
 ...
 * 
 * The optional primkey-field may be present in the descriptor if the entity's
 * persistence-type is Container.
 * 
 * The optional cmp-version element may be present in the descriptor if the
 * entity's persistence-type is Container. If the persistence-type is Container
 * and the cmp-version element is not specified, its value defaults to 2.x.
 * 
 * The optional home and remote elements must be specified if the entity bean
 * cmp-version is 1.x.
 * 
 * The optional home and remote elements must be specified if the entity bean
 * has a remote home and remote interface.
 * 
 * The optional local-home and local elements must be specified if the entity
 * bean has a local home and local interface.
 * 
 * Either both the local-home and the local elements or both the home and the
 * remote elements must be specified.
 * 
 ...
 * 
 * The other elements that are optional are "optional" in the sense that they
 * are omitted if the lists represented by them are empty.
 * 
 * At least one cmp-field element must be present in the descriptor if the
 * entity's persistence-type is Container and the cmp-version is 1.x, and none
 * must not be present if the entity's persistence-type is Bean.
 * 
 * 
 * 
 */
public class EntityBeanTypeImpl<T> implements Child<T>, EntityBeanType<T> {
	...
	public List<IconType<EntityBeanType<T>>> getIconList() {
		...
		for (Node node : nodeList) {
			IconType<EntityBeanType<T>> type = new IconTypeImpl<EntityBeanType<T>>(
	...
	public EnvEntryType<EntityBeanType<T>> envEntry() {
		return new EnvEntryTypeImpl<EntityBeanType<T>>(this, "env-entry",
				childNode);
	}
	...
	public List<EnvEntryType<EntityBeanType<T>>> getEnvEntryList() {
		...
		for (Node node : nodeList) {
			EnvEntryType<EntityBeanType<T>> type = new EnvEntryTypeImpl<EntityBeanType<T>>(
	...
	public List<EjbRefType<EntityBeanType<T>>> getEjbRefList() {
		...
		for (Node node : nodeList) {
			EjbRefType<EntityBeanType<T>> type = new EjbRefTypeImpl<EntityBeanType<T>>(
	...
	public EjbLocalRefType<EntityBeanType<T>> ejbLocalRef() {
		return new EjbLocalRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<EjbLocalRefType<EntityBeanType<T>>> getEjbLocalRefList() {
		...
		for (Node node : nodeList) {
			EjbLocalRefType<EntityBeanType<T>> type = new EjbLocalRefTypeImpl<EntityBeanType<T>>(
	...
	public ResourceRefType<EntityBeanType<T>> resourceRef() {
		return new ResourceRefTypeImpl<EntityBeanType<T>>(this, "resource-ref",
				childNode);
	}
	...
	public List<ResourceRefType<EntityBeanType<T>>> getResourceRefList() {
		...
		for (Node node : nodeList) {
			ResourceRefType<EntityBeanType<T>> type = new ResourceRefTypeImpl<EntityBeanType<T>>(
	...
	public ResourceEnvRefType<EntityBeanType<T>> resourceEnvRef() {
		return new ResourceEnvRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<ResourceEnvRefType<EntityBeanType<T>>> getResourceEnvRefList() {
		...
		for (Node node : nodeList) {
			ResourceEnvRefType<EntityBeanType<T>> type = new ResourceEnvRefTypeImpl<EntityBeanType<T>>(
	...
	public MessageDestinationRefType<EntityBeanType<T>> messageDestinationRef() {
		return new MessageDestinationRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<MessageDestinationRefType<EntityBeanType<T>>> getMessageDestinationRefList() {
		...
		for (Node node : nodeList) {
			MessageDestinationRefType<EntityBeanType<T>> type = new MessageDestinationRefTypeImpl<EntityBeanType<T>>(
	...
	public PersistenceContextRefType<EntityBeanType<T>> persistenceContextRef() {
		return new PersistenceContextRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<PersistenceContextRefType<EntityBeanType<T>>> getPersistenceContextRefList() {
		...
		for (Node node : nodeList) {
			PersistenceContextRefType<EntityBeanType<T>> type = new PersistenceContextRefTypeImpl<EntityBeanType<T>>(
	...
	public PersistenceUnitRefType<EntityBeanType<T>> persistenceUnitRef() {
		return new PersistenceUnitRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<PersistenceUnitRefType<EntityBeanType<T>>> getPersistenceUnitRefList() {
		...
		for (Node node : nodeList) {
			PersistenceUnitRefType<EntityBeanType<T>> type = new PersistenceUnitRefTypeImpl<EntityBeanType<T>>(
	...
	public LifecycleCallbackType<EntityBeanType<T>> postConstruct() {
		return new LifecycleCallbackTypeImpl<EntityBeanType<T>>(this,
	...
	public List<LifecycleCallbackType<EntityBeanType<T>>> getPostConstructList() {
		...
		for (Node node : nodeList) {
			LifecycleCallbackType<EntityBeanType<T>> type = new LifecycleCallbackTypeImpl<EntityBeanType<T>>(
	...
	public ServiceRefType<EntityBeanType<T>> serviceRef() {
		return new ServiceRefTypeImpl<EntityBeanType<T>>(this, "service-ref",
				childNode);
	}
	...
	public List<ServiceRefType<EntityBeanType<T>>> getServiceRefList() {
		...
		for (Node node : nodeList) {
			ServiceRefType<EntityBeanType<T>> type = new ServiceRefTypeImpl<EntityBeanType<T>>(
	...
	// isComplexType: false maxOccurs: - isAttribute: false
	// -------------------------------------------------------------------------------------||
	public EntityBeanType<T> setPersistenceType(
			PersistenceTypeType persistenceType) {
	...
	public PersistenceTypeType getPersistenceType() {
		return PersistenceTypeType.getFromStringValue(childNode
				.textValue("persistence-type"));
	}
	...
	public CmpVersionType getCmpVersion() {
		return CmpVersionType.getFromStringValue(childNode
				.textValue("cmp-version"));
	}
	...
	public CmpFieldType<EntityBeanType<T>> cmpField() {
		return new CmpFieldTypeImpl<EntityBeanType<T>>(this, "cmp-field",
				childNode);
	}
	...
	public List<CmpFieldType<EntityBeanType<T>>> getCmpFieldList() {
		...
		for (Node node : nodeList) {
			CmpFieldType<EntityBeanType<T>> type = new CmpFieldTypeImpl<EntityBeanType<T>>(
					this, "cmp-field", childNode, node);
			list.add(type);
		}
		return list;
	}
	...
	public SecurityRoleRefType<EntityBeanType<T>> securityRoleRef() {
		return new SecurityRoleRefTypeImpl<EntityBeanType<T>>(this,
	...
	public List<SecurityRoleRefType<EntityBeanType<T>>> getSecurityRoleRefList() {
		...
		for (Node node : nodeList) {
			SecurityRoleRefType<EntityBeanType<T>> type = new SecurityRoleRefTypeImpl<EntityBeanType<T>>(
	...
	public SecurityIdentityType<EntityBeanType<T>> securityIdentity() {
		...
		SecurityIdentityType<EntityBeanType<T>> securityIdentity = new SecurityIdentityTypeImpl<EntityBeanType<T>>(
	...
	public List<QueryType<EntityBeanType<T>>> getQueryList() {
		...
		for (Node node : nodeList) {
			QueryType<EntityBeanType<T>> type = new QueryTypeImpl<EntityBeanType<T>>(
					this, "query", childNode, node);
			list.add(type);
		}
		return list;
	}
