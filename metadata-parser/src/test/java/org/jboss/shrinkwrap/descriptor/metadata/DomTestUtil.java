...
public class DomTestUtil {
	 ...
	 * @param element contains the parsed element information.
	 * @param xmlFragment the original XML element information.
	 */
	public static void assertElement(final MetadataElement element, final String xmlFragment) {
		final String tmpStr = xmlFragment.replace("\"", "").replaceAll("/>|>" , "").replaceAll("</?[a-z]*:element>?", "").trim();
	...
	public static void assertClassAttribute(final MetadataElement element, final String xmlFragment) {
		final String tmpStr = xmlFragment.replace("\"", "").replaceAll("/>|>" , "").replaceAll("</?[a-z]*:attribute>?", "").trim();
		final String[] attributes = tmpStr.split(" ", -1);
		for (String attribute: attributes) {
			final String[] items = attribute.split("=", -1);
			if (items.length == 2) {
	...
	/**
	 * Tests the given xmlFragement against the given MetadataElement.
	 * @param group contains the parsed element information.
	 * @param xmlFragment the original XML element information.
	 */
	public static void assertGroup(final MetadataElement group, final String xmlFragment) {
		final String tmpStr = xmlFragment.replace("\"", "").replaceAll("/>|>" , "").replaceAll("</?[a-z]*:group>?", "").trim();
		final String[] attributes = tmpStr.split(" ", -1);
		for (String attribute: attributes) {
			final String[] items = attribute.split("=", -1);
			if (items.length == 2) {
