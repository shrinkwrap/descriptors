 ...
 */
package org.jboss.shrinkwrap.descriptor.spi.node.query.matcher;
...
/**
 * Algorithm for retrieving nodes matching
...
public class RelativeNodeMatcher implements NodeMatcher
{
   ...
   private List<Node> findMatch(final Node start, final List<Pattern> currentPatternSequence, final List<Pattern> entirePatternSequence)
   {
      if (currentPatternSequence.isEmpty())
      {
      ...
      final Pattern pattern = currentPatternSequence.get(0);
      ...
      if (pattern.matches(start))
      {
         ...
         if (currentPatternSequence.size() == 1)
         {
         ...
         for (final Node child : start.getChildren())
         {
            ...
            final List<Pattern> remainingPatternsSequence = currentPatternSequence.subList(1, currentPatternSequence.size());

            // Recursion point
            matchedNodes.addAll(findMatch(child, remainingPatternsSequence, entirePatternSequence));
         }

      }

      // Apply whole pattern sequence starting from 
